...
public class LoginServiceImpl implements LoginService {
        ...
        while (true) {
            ...
            try {
                ...
                if (ResultEnum.SUCCESS.getCode().equals(status)) {
                    if (processLoginInfo(result)) { // 处理结果
                        core.setAlive(true);
                        return true;
                    } else {
                        return false;
                    ...
                    LOG.info(ResultEnum.WAIT_CONFIRM.getMsg());
                    SleepUtils.sleep(1000);
                } else if (ResultEnum.WAITING.getCode().equals(status)) {
                    LOG.info(ResultEnum.WAITING.getMsg());
                    SleepUtils.sleep(1000);
                } else if (ResultEnum.EXPIRED.getCode().equals(status)) {
                    LOG.info(ResultEnum.EXPIRED.getMsg());
                    break;
                } else {
                ...
                LOG.error("Exception in waiting scan", e);
                break;
            }
        }
        return false;
        ...
        try {
            ...
            for (int i = 0; i < chatSetArray.length; i++) {
                if (chatSetArray[i].indexOf("@@") != -1) {
                    core.getGroupChatRoomNameSet().add(chatSetArray[i]);
                }
            }
    ...
    public void startReceiving() {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                LOG.info("startReceiving exited.");
            }
        }).start();
        ...
        try {
            ...
            for (Iterator<?> iterator = member.iterator(); iterator.hasNext(); ) {
                ...
                if ((o.getInteger("VerifyFlag") & 8) != 0) { // 公众号/服务号
                    ...
                    core.getGroupChatRoomNameSet().add(o.getString("UserName"));
                ...
                core.getContactMap().put(o.getString("UserName"), o);
            }
            return;
        } catch (Exception e) {
            LOG.error(e.getMessage(), e);
    ...
    public void WebWxBatchGetContact() {
        String url = String.format(URLEnum.WEB_WX_BATCH_GET_CONTACT.getUrl(),
                ...
                core.getLoginInfo().get(StorageLoginInfoEnum.pass_ticket.getKey()));
        Map<String, Object> paramMap = core.getParamMap();
        paramMap.put("Count", core.getGroupChatRoomNameSet().size());
        List<Map<String, String>> list = new ArrayList<Map<String, String>>();
        for (String name : core.getGroupChatRoomNameSet()) {
        ...
        try {
            ...
            for (int i = 0; i < contactList.size(); i++) { // 群好友
                if (contactList.getJSONObject(i).getString("UserName").indexOf("@@") > -1) { // 群
                    ChatRoom room = new ChatRoom(contactList.getJSONObject(i), 100);
                    core.getChatRoomMap().put(room.getName(), room); // 更新群聊天室
                    core.getContactMap().put(contactList.getJSONObject(i).getString("UserName"), contactList.getJSONObject(i));
                }
            }
        } catch (Exception e) {
            LOG.info(e.getMessage());
