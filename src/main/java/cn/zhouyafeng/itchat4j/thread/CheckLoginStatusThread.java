 ...
 *
 */
public class CheckLoginStatusThread extends Thread {
	...
	public void run() {
		while (core.isAlive()) {
			long diff = (System.currentTimeMillis() - core.getLastNormalRetcodeTime())/1000;
			if (diff > 60) { // 超过30秒，判为离线
				core.setAlive(false);
				LOG.info("微信已离线");
			}
			LOG.info("lag:"+diff+", msgs:{}, rooms:{}",
					core.getMsgList().size(), core.getChatRoomMap().size());
			SleepUtils.sleep(10 * 1000); // 休眠10秒
		}
		LOG.info("CheckLoginStatusThread exited.");
	}
